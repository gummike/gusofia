package com.tongbanjie.gusofia.nio_socket;
...
public class SocketServerNIO {
     ...
     * 日志
     */
    private static final Log LOGGER = LogFactory.getLog(SocketServerNIO.class);
            ...
            while (true) {
                ...
                while (selecionKeys.hasNext()) {
                    ...
                    if (readyKey.isValid() && readyKey.isAcceptable()) {
                        SocketServerNIO.LOGGER.info("======channel通道已经准备好=======");
                        /*
                        ...
                        SocketServerNIO.LOGGER.info("======socket channel 建立连接=======");
                    } else if (readyKey.isValid() && readyKey.isReadable()) {
                        SocketServerNIO.LOGGER.info("======socket channel 数据准备完成，可以去读==读取=======");
                        readSocketChannel(readyKey);
                    }
                }
            }
        } catch (Exception e) {
            SocketServerNIO.LOGGER.error(e.getMessage(), e);
        } finally {
            serverSocket.close();
        }
    }
    ...
    private static void readSocketChannel(SelectionKey readyKey) throws Exception {
        ...
        try {
            ...
            SocketServerNIO.LOGGER.error(e.getMessage());
        ...
        if (realLen == -1) {
            SocketServerNIO.LOGGER.warn("====缓存区没有数据？====");
            return;
        }
        ...
        if (message.indexOf("over") != -1) {
            ...
            contextBytes.clear();
            SocketServerNIO.LOGGER.info("端口:" + resourcePort + "客户端发来的信息======message : " + message);
            ...
            SocketServerNIO.LOGGER.info("端口:" + resourcePort + "客户端信息还未接受完，继续接受======message : " + message);
